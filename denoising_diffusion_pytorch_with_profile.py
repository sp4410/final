...
class GaussianDiffusionScript(nn.Module):
    def __init__(
        self,
        model: torch.jit.RecursiveScriptModule,  # change the type hint to RecursiveScriptModule
        channels:int,
        image_size: int,
        timesteps: int,
        ...
        self.model = model  # assign the RecursiveScriptModule to the model attribute
...
class Trainer(object):
    def __init__(
        ...
        *,
        ...
        #Wasnt commented out orignially
        
       
        if self.accelerator.is_main_process:
            if is_scripted_model==False:
                self.ema= EMA(diffusion_model,beta=ema_decay,update_every=ema_update_every)
            else:
                for param in diffusion_model.parameters():
                    if not param.is_leaf:
                        param.detach_()
                unwrapped_model=torch.jit_unwrap_optional(diffusion_model)
                self.ema=EMA(unwrapped_model, beta = ema_decay, update_every = ema_update_every)
    ...
    def train(self,profiler):
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            while self.step < self.train_num_steps:
                ...
                for _ in range(self.gradient_accumulate_every):
                    with record_function("data_load_to_device"):
                        data = next(self.dl).to(device)
                    
                    with record_function("data_forward"):
                        with self.accelerator.autocast():
                            loss = self.model(data)
                            loss = loss / self.gradient_accumulate_every
                            total_loss += loss.item()
                            #if total_loss.isinf==True:
                            #    return
                            self.loss_list.append(total_loss)   #Added by Me
                    
                    with record_function("data_backward"):
        ...
        return self.loss_list #Added by me
